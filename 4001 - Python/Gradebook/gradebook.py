...
for col in range(len(gradebook[0])):
    AssignmentSum = 0
    for row in range(len(gradebook)):
        AssignmentSum += gradebook[row][col]
    print("Assignment", col+1,":", format((AssignmentSum / len(gradebook)), '.2f'), sep='')
    AssignmentAverage = 0

print("\nStudent Averages:")
for col in range(len(gradebook)):
    StudentAverage = 0
    for row in range(len(gradebook[0])):
        StudentAverage += gradebook[col][row]
    print("Student", row+1,":", format((StudentAverage / len(gradebook[0])), '.2f'), sep='')
